...
function addCardInteraction(el){
  ...
  el.addEventListener('pointerdown', e=>{
    ...
    const moveHandler = e=>{
      ...
      el.style.transform = `translate(${currentX}px, ${currentY}px) rotate(${currentX/12}deg)`;
    };
    const upHandler = e=>{
      dragging=false;
...
function swipeCard(dir, el){
  ...
  if(correct){
    rightCount++; rightCountEl.textContent=rightCount;
    rightWords.push(top.word);
  } else {
    wrongCount++; wrongCountEl.textContent=wrongCount;
    wrongWords.push(top.word);
  }
...
function showGameEnd(){
  ...
  finalResultsEl.innerHTML = `
    <h3>Dein Ergebnis:</h3>
    <div>Richtig gezogen:</div>
    <ul>${rightWords.map(w=>`<li class="right">${w}</li>`).join('')}</ul>
    <div>Falsch gezogen:</div>
    <ul>${wrongWords.map(w=>`<li class="wrong">${w}</li>`).join('')}</ul>
  `;
}
